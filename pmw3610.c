...
// QMK "custom" pointing device driver entry points.
// pointing_device.c (custom mode) calls these exact names via weak-symbol
// override, so no separate driver struct or name-mangled registration is
// needed here.
